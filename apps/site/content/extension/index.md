...
- **[Language support](/extension/language-support)** — `.sdoc` files are
  served by the Svelte language server: completion, hover, diagnostics,
  go-to-definition, and formatting, plus sdocs-specific extras like
  meta-field autocomplete and doc-level diagnostics.
...
with the component import, `meta`, and a `Default(args)` snippet. If the doc
already exists, it opens instead.
