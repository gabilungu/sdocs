...
The plugin accepts the same options as [`sdocs.config.js`](./configuration.md). Options passed here override any config file.
...
| `sidebarConfig` | `{ order?, open? }` | Sidebar ordering & initial open state. See [sidebar](../features/sidebar.md). |

See [types](../api/types.md) for `DocEntry`.
...
- [Configuration reference](./configuration.md) — full list of plugin options
- [Standalone CLI](./standalone-cli.md) — if you don't need to embed
