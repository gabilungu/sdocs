...
const TAG_SHOWCASE: Record<string, string> = {
	...
	NOTES: [
		...
		'\t- deprecated: Being replaced by ActionButton.',
		'\t- Just a remark, with no status.',
		'[/NOTES]',
		'```',
		'',
		'The status is `bug`, `deprecated`, `wip` or `ready`; leave it off for a plain remark. The sidebar shows the worst note at or under a row, ranked `bug` → `deprecated` → `wip` → *no status* → `ready` — a remark outranks reassurance.',
		'',
