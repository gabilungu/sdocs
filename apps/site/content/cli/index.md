...
The sdocs CLI runs the [Explorer](/explorer) as its own dev server — no host
app required — and builds it into a deployable static site.
...
Nothing is added to your project — npm fetches sdocs into its cache and starts
...
For everything the CLI can do, see the [command reference](/cli/commands):
...
All commands read [`sdocs.config.js`](/explorer/configuration) from the
current directory.
