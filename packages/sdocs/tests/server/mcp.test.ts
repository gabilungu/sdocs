...
describe("MCP handler", () => {
  ...
  it("lists the fourteen tools, reads before writes", async () => {
    ...
    const names = (result as { tools: { name: string }[] }).tools.map(
      (t) => t.name,
    );
  ...
  it("serves the visual testing guide as a resource", async () => {
    const list = (await rpc("resources/list")).result as {
      resources: { uri: string }[];
    };
    expect(list.resources.map((r) => r.uri)).toContain(
      "sdocs://visual-testing-guide",
    );
    const read = (
      await rpc("resources/read", { uri: "sdocs://visual-testing-guide" })
...
`,
    ...
    const structured = (
      await callTool("scaffold_component_doc", { componentPath: component })
    ).structuredContent as Record<string, any>;
...
`,
    ...
    const api = (
      await callTool("get_component_api", { componentPath: component })
    ).structuredContent as Record<string, any>;
    ...
    expect(api.methods).toMatchObject([
      { name: "clear", description: "Clears the value" },
    ]);
  ...
  it("maps the project docs with list_docs", async () => {
    ...
    process.chdir(dir);
    try {
      const result = (await callTool("list_docs")).structuredContent as Record<
        string,
        any
      >;
      ...
      expect(result.docs[0]).toMatchObject({
        valid: true,
        entities: [
          { kind: "SHOWCASE", title: "Forms / Button", components: ["Button"] },
        ],
      });
  ...
  describe("search_docs", () => {
    ...
    const search = async (query: string) =>
      (await callTool("search_docs", { query })).structuredContent as Record<
        string,
        any
      >;
    ...
    it("finds a page by the text of its note", async () => {
      ...
      expect(found.results).toMatchObject([
        {
          title: "Display / Badge",
          matched: ["note: Being replaced by Chip in v4."],
        },
      ]);
    ...
    it("sweeps by type with no query at all", async () => {
      const found = await inProject(
        async () =>
          (await callTool("search_docs", { type: "bug" }))
            .structuredContent as Record<string, any>,
      );
      expect(found.results).toHaveLength(1);
      expect(found.results[0]).toMatchObject({
    ...
    it("finds a note written without a type under 'none'", async () => {
      const found = await inProject(
        async () =>
          (await callTool("search_docs", { type: "none" }))
            .structuredContent as Record<string, any>,
      );
      expect(found.results).toHaveLength(1);
    ...
    it("requires both when both are given", async () => {
      const both = async (query: string, type: string) =>
        (await callTool("search_docs", { query, type }))
          .structuredContent as Record<string, any>;
  ...
  describe("resolve_visual_target", () => {
    ...
    it("resolves a qualified name to a preview route and the files behind it", async () => {
      await inProject(async () => {
        const r = (
          await callTool("resolve_visual_target", { target: "Button / Sizes" })
        ).structuredContent as Record<string, any>;
        ...
        expect(r.resolved.previewRoute).toMatch(
          /^\/@sdocs\/preview\/[^/]+\/x-sizes$/,
        );
        expect(r.resolved.builtPreviewPath).toMatch(
          /^\/previews\/[^/]+\/x-sizes\.html$/,
        );
    ...
    it("reports the component file to edit, not just the route to look at", async () => {
      await inProject(async () => {
        const r = (
          await callTool("resolve_visual_target", { target: "Button" })
        ).structuredContent as Record<string, any>;
    ...
    it("resolves a stage route to that stage", async () => {
      await inProject(async () => {
        const listed = (await callTool("list_docs"))
          .structuredContent as Record<string, any>;
        const exampleRoute = listed.docs[0].entities[0].examples[0]
          .route as string;
        const r = (
          await callTool("resolve_visual_target", { target: exampleRoute })
        ).structuredContent as Record<string, any>;
        expect(r.resolved?.name).toBe("Sizes");
        expect(r.resolved?.kind).toBe("example");
      });
    ...
    it("does not fuzzy-match a route that addresses nothing", async () => {
      ...
      await inProject(async () => {
        const r = (
          await callTool("resolve_visual_target", { target: "/no/such/route" })
        ).structuredContent as Record<string, any>;
    ...
    it("reports a LAYOUT's .sdoc line and its sole component root", async () => {
      ...
      try {
        const r = (
          await callTool("resolve_visual_target", { target: "Inbox / Content" })
        ).structuredContent as Record<string, any>;
    ...
    it("explains that a DOC route has no stage instead of returning a bare null", async () => {
      ...
      try {
        const r = (
          await callTool("resolve_visual_target", {
            target: "/docs/guides/intro",
          })
        ).structuredContent as Record<string, any>;
        expect(r.resolved).toBe(null);
  ...
  it("compiles every stage with check_docs and reports what breaks", async () => {
    ...
    process.chdir(dir);
    try {
      const result = (await callTool("check_docs")).structuredContent as Record<
        string,
        any
      >;
      ...
      expect(result.problems.some((p: any) => p.stage === "Unclosed")).toBe(
        true,
      );
    } finally {
      process.chdir(prev);
  ...
  it("reports documentation coverage with check_coverage", async () => {
    ...
    process.chdir(dir);
    try {
      const result = (await callTool("check_coverage"))
        .structuredContent as Record<string, any>;
...
describe("validate_sdoc reports where a title will be served", () => {
  ...
  it("does not split CamelCase — the trap, made visible", async () => {
    const [entity] = await routesOf(
      '[SHOWCASE title="IconButton"]\n[/SHOWCASE]',
    );
  ...
  it("slugifies each segment of a nested title", async () => {
    const [entity] = await routesOf(
      '[SHOWCASE title="Form Controls / Icon Button"]\n[/SHOWCASE]',
    );
  ...
  it("carries the section prefix", async () => {
    const [entity] = await routesOf(
      '[DOC title="@guides/Getting Started"]\n[/DOC]',
    );
  ...
  it("honours an explicit slug override on the leaf", async () => {
    const [entity] = await routesOf(
      '[SHOWCASE title="IconButton" slug="icon-button"]\n[/SHOWCASE]',
    );
...
describe("get_authoring_guide({ section }) and get_changelog({ since })", () => {
  ...
  it("matches a heading loosely, so the caller need not know its punctuation", async () => {
    expect(await textOf("get_authoring_guide", { section: "notes" })).toContain(
      "[NOTES]",
    );
  });
  ...
  it("answers an unknown section with the list of headings", async () => {
    const reply = await textOf("get_authoring_guide", {
      section: "nothing-like-this",
    });
  ...
  it("returns the whole changelog with no version, and says so for an unknown one", async () => {
    ...
    expect(await textOf("get_changelog", { since: "9.9.9" })).toContain(
      'No release "9.9.9"',
    );
  });
});
...
describe("the blocks over MCP", () => {
  ...
  it("reports todos and glossary terms, not just notes", async () => {
    ...
    const entity = await inDir(
      dir,
      async () =>
        ((await callTool("list_docs")).structuredContent as any).docs[0]
          .entities[0],
    );
    ...
    expect(entity.glossary).toEqual([
      {
        term: "Badge",
        definition: "a small count or label attached to something.",
      },
    ]);
  });
  ...
  it("searches todo text and glossary definitions", async () => {
    ...
    const search = async (query: string) =>
      (
        (await callTool("search_docs", { query })).structuredContent as any
      ).results.flatMap((r: { matched: string[] }) => r.matched);
    expect(await inDir(dir, () => search("ceiling"))).toContain(
      "todo: Decide the ceiling",
    );
    expect(await inDir(dir, () => search("count or label"))).toContain(
      "definition of Badge",
    );
  });
  ...
  it("sets a status, and removes it again byte-for-byte", async () => {
    ...
    expect(readFileSync(file, "utf-8")).toContain(
      '[COMPONENT component={Badge} status="review"]',
    );
    await set(null);
