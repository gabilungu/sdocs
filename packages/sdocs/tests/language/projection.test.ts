...
import {
  projectSdoc,
  projectSdocBlocks,
} from "../../src/lib/language/projection.js";
...
const ownersOf = (
  blocks: ReturnType<typeof projectSdocBlocks>,
  lineCount: number,
): number[] => {
...
describe("projectSdoc", () => {
  ...
  it("produces text the Svelte compiler accepts", () => {
    expect(() =>
      compile(projection.text, { generate: false } as never),
    ).not.toThrow();
  });
});
...
`;
  ...
  it("merges file and block scripts into one component script", () => {
    ...
    const closeLine = sourceLines.findIndex(
      (l, i) => i > itemsLine && l.includes("</script>"),
    );
  ...
  it("a style-only block keeps the file script closed and unextended", () => {
    ...
    const openerLine = sourceLines.findIndex((l) =>
      l.includes('title="StyleOnly"'),
    );
...
describe("projectSdocBlocks: single-line tags (review regressions)", () => {
  const compileOk = (text: string) =>
    expect(compile(text, { generate: "client" }).js.code).toBeTruthy();
  ...
  it("keeps a one-liner block script with NO file script", () => {
    ...
    expect(bp.text).toContain(
      "<script>  let count = $state(7);</script>{#snippet",
    );
    compileOk(bp.text);
  });
...
`;
  ...
  it("entity script lines are owned by the per-entity doc, exactly once", () => {
    ...
    const sharedLine = sourceLines.findIndex((l) =>
      l.includes("const shared = [1, 2];"),
    );
...
`;
    ...
    expect(blocks.every((b) => (b.extraOwnedLines?.length ?? 0) === 0)).toBe(
      true,
    );
    compileOk(doc.text);
...
`;
    ...
    expect(lines[openLine].indexOf("let b = 2;")).toBe(
      sourceLines[openLine].indexOf("let b = 2;"),
    );
...
`;
    ...
    expect(lines[line].indexOf("const shift")).toBe(
      sourceLines[line].indexOf("const shift"),
    );
    compileOk(bp.text);
  });
});
...
`;
    ...
    const owner = ownersOf(blocks, sourceLines.length);
    const importLine = sourceLines.findIndex((l) =>
      l.includes("import Notice"),
    );
...
`;
    ...
    const owner = ownersOf(blocks, sourceLines.length);
    const localLine = sourceLines.findIndex((l) =>
      l.includes("<b>{local}</b>"),
    );
...
describe("projectSdocBlocks: recomposed content spans (column-preserving)", () => {
  ...
  it("an inner script opener sharing content records the span at authored columns", () => {
    ...
    expect(bp.contentSpans?.[openLine]).toEqual({
      start: openCol,
      end: openCol + "let b = 2;".length,
    });
    ...
    expect(bp.contentSpans?.[closeLine]).toEqual({
      start: 0,
      end: "\t\t\tlet c = 3;".length,
    });
    compileOk(bp.text);
  ...
  it("a single-line PAGE entity style records its span (no graft, indent preserved)", () => {
    ...
    expect(pg.contentSpans?.[line]).toEqual({
      start: col,
      end: col + ".x { color: red; }".length,
    });
    compileOk(pg.text);
  });
});
...
`;
    ...
    const lines = projection.text.split("\n");
    expect(
      lines[sourceLines.findIndex((l) => l.includes("const answer"))],
    ).toBe("");
...
/**
 * CommonMark lets a fence open inside a blockquote, and GitHub callouts make
 * that the ordinary way to show code in an aside. Read as prose instead, the
 * fenced line hands Svelte an object literal and the whole page stops
 * compiling — which is how this shipped into the CLI docs.
 */
describe("code fences inside blockquotes", () => {
  const wrap = (body: string) =>
    `[DOC title="Guides / Build"]\n\n${body}\n\n[/DOC]\n`;
  const project = (src: string) => projectSdoc(scanSdoc(src, "test.sdoc"));
  const compiles = (src: string) =>
    expect(() =>
      compile(project(src).text, { generate: "client" }),
    ).not.toThrow();

  it("treats a fence in a callout as code, not as an expression", () => {
    compiles(
      wrap(
        [
          "\t> [!IMPORTANT]",
          "\t> Point sdocs somewhere of its own:",
          "\t>",
          "\t> ```js",
  ...
  it("does not swallow the document when the quoted fence is unclosed", () => {
    // The blockquote ends, so the fence ends with it — the heading after it
    // is prose again, not code.
    const src = wrap(
      ["\t> ```js", "\t> const a = 1;", "", "\t## Still a heading"].join("\n"),
    );
    compiles(src);
    expect(project(src).text).toContain("## Still a heading");
  });

  it("still masks an expression inside a quoted fence", () => {
    const src = wrap(
      ["\t> ```js", "\t> const a = { b: 1 };", "\t> ```"].join("\n"),
    );
