...
export type SubBlockKind =
  "preview" | "example" | "prose" | "notes" | "todo" | "glossary";
...
export const TEXT_BLOCK_KINDS: readonly SubBlockKind[] = [
  "prose",
  "notes",
  "todo",
];
...
export function textBlockCloser(kind: SubBlockKind): string {
  const tag = Object.entries(TEXT_BLOCK_TAG_KINDS).find(
    ([, k]) => k === kind,
  )?.[0];
  return `[/${tag}]`;
}
...
export function subBlockKindOf(name: string): SubBlockKind | null {
  return (
    UPPERCASE_SUB_BLOCK_TAG_KINDS[name] ??
    SUB_BLOCK_TAG_KINDS[name.toLowerCase()] ??
    null
  );
}
...
function scanOpenerAttrs(
  ...
  while (i < source.length) {
    ...
    if (!nameMatch) {
      ...
      const hint =
        ch === ">"
          ? " Block openers close with ']', not '>' — the attributes are HTML-like, the brackets are not."
          : "";
      errors.push({
        code: "attr-syntax",
...
export function scanSdoc(source: string): SdocFile {
  ...
  function skipComment(startLi: number): number {
    const startOffset =
      lines[startLi].start + lines[startLi].text.indexOf("<!--");
  ...
  function scanOpener(
    ...
    const attrs: Attrs = {};
    const end = scanOpenerAttrs(
      source,
      bracketStart + 1 + kindLen,
      attrs,
      errors,
    );
  ...
  function captureBody(
    ...
    const bodyStart =
      startLi < lines.length ? lines[startLi].start : source.length;
  ...
  function scanSubBlockBody(
    ...
    const bodyEnd =
      closerLi > fromLi
        ? lines[closerLi - 1].end
        : (lines[fromLi]?.start ?? source.length);
    let script: TagBlock | null = null;
    let style: TagBlock | null = null;
    ...
    while (j < closerLi) {
      ...
      if (startsTag(trimmed, "script")) {
        errors.push({
          ...
          span: {
            start: lines[j].start + lines[j].text.indexOf("<script"),
            end: lines[j].end,
          },
        });
      }
    ...
    while (i < lines.length) {
      ...
      if (startsTag(trimmed, "script")) {
        if (sawContent || entity.script) {
          errors.push({
            ...
            span: {
              start: line.start + line.text.indexOf("<script"),
              end: line.end,
            },
          });
          i++;
        ...
        const captured = captureBoundedTag(
          "script",
          i,
          closerBound(closerLi),
          `[/${entity.kind}]`,
        );
        if (!captured) {
      ...
      if (textKind) {
        ...
        entity.blocks.push({
          ...
          openerSpan: {
            start: line.start + line.text.indexOf("["),
            end: line.end,
          },
          span: {
            start: line.start + line.text.indexOf("["),
            end: captured.closerSpan.end,
          },
        });
        i = captured.nextLi;
        continue;
      ...
      const span = {
        start: line.start + line.text.indexOf(trimmed[0]),
        end: line.end,
      };
      ...
      errors.push({
        code: "text-outside-blocks",
        message:
          "Text inside [SHOWCASE] must be inside a [component] or [example] block.",
        span,
      });
      i++;
    }
  ...
  function scanDocBody(startLi: number, entity: Entity): number {
    ...
    const demoteMisplacedStyle = () => {
      ...
      if (!styleMisplacedReported) {
        ...
        errors.push({
          code: "entity-style-position",
          message:
            "An entity <style> must be the last content of its [DOC] body.",
          span: entity.style.span,
        });
      }
    ...
    while (i < lines.length) {
      ...
      const docKind =
        token && !token.closer ? subBlockKindOf(token.name) : null;
    ...
    if (startsTag(trimmed, "script")) {
      ...
      if (script) {
        ...
        errors.push({
          ...
          message:
            "<script> must be at the top of the file, before any entity.",
          span: captured.block.span,
        });
      } else {
        script = captured.block;
    ...
    if (token && !token.closer && isEntityKind(token.name)) {
      if (style) {
        errors.push({
          code: "style-position",
          message:
            "<style> must be at the bottom of the file, after all entities.",
          span: { start: line.start, end: line.end },
        });
      }
      ...
      if (token.name === "SHOWCASE") {
        ...
        if (!captured) {
          ...
          entity.body = source.slice(
            opener.nextLi < lines.length
              ? lines[opener.nextLi].start
              : source.length,
          );
          entity.span.end = source.length;
