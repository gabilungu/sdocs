...
beforeAll(async () => {
  ...
  writeFileSync(
    ...
    '{"name":"bare","private":true,"type":"module"}\n',
  );
  ...
  symlinkSync(
    join(REPO, "node_modules/svelte"),
    join(fixture, "node_modules/svelte"),
    "junction",
  );
...
describe("server bundle in an installed-shaped directory", () => {
  it("starts, serves a corpus file, and settles at zero diagnostics", async () => {
    ...
    // Errors and warnings only. TypeScript also emits severity-4 hints —
    // "implicitly has an 'any' type, but a better type may be inferred" on
    // an untyped snippet parameter in a .js file, say — which VS Code draws
    // as three faint dots and keeps out of the Problems panel. Asserting on
    // those makes this break every time upstream adds a suggestion, and
    // what it exists to catch is the broken-bundle error.
    expect(
      last.diagnostics
        .filter((d) => (d.severity ?? 1) <= 2)
        .map(
          (d) =>
            `${d.range.start.line + 1}: ${String(d.message).split("\n")[0]}`,
        ),
    ).toEqual([]);
  });
  ...
  it("still reports real errors at authored lines (service is alive, not silent)", async () => {
    ...
    await client.changeDoc(
      uri,
      2,
      source.replace("{...args}", "{...args} title={nopeVar}"),
    );
    const publish = await client.waitForDiagnostics(uri, (p) =>
      p.diagnostics.some((d) => String(d.message).includes("nopeVar")),
    );
    const hit = publish.diagnostics.find((d) =>
      String(d.message).includes("nopeVar"),
    )!;
