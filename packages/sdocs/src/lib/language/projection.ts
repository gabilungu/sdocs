...
export type ProjectedLineKind =
  "verbatim" | "blank" | "wrapper" | "masked" | "recomposed";
...
function maskMarkdownLine(
  ...
  if (stepFence(state, line).inside) return { text: "", masked: true };
...
export function projectSdoc(file: SdocFile): SdocProjection {
  ...
  const wrapBlock = (
    ...
    const openLast = lineOfOffset(
      starts,
      Math.max(openerSpan.start, openerSpan.end - 1),
    );
    ...
    if (bodySpan.end > bodySpan.start) {
      if (maskBody) {
        ...
        const last = lineOfOffset(
          starts,
          Math.max(bodySpan.start, bodySpan.end - 1),
        );
    ...
    const closeLine = lineOfOffset(
      starts,
      Math.max(closerLineOf.start, closerLineOf.end - 1),
    );
  ...
  const wrapDocWithExamples = (entity: Entity, e: number) => {
    ...
    const openLast = lineOfOffset(
      starts,
      Math.max(entity.openerSpan.start, entity.openerSpan.end - 1),
    );
    ...
    entity.blocks.forEach((block: SubBlock, b: number) => {
      ...
      const bOpenLast = lineOfOffset(
        starts,
        Math.max(block.openerSpan.start, block.openerSpan.end - 1),
      );
    ...
    const closeLine = lineOfOffset(
      starts,
      Math.max(entity.span.start, entity.span.end - 1),
    );
    let proseTo = closeLine - 1;
    if (entity.style) {
      proseTo = Math.min(
        proseTo,
        lineOfOffset(starts, entity.style.span.start) - 1,
      );
    }
...
export function projectSdocBlocks(file: SdocFile): SdocBlockProjection[] {
  ...
  const emitStyle = (
    ...
    const close = lineOfOffset(
      starts,
      Math.max(style.span.start, style.span.end - 1),
    );
    if (style.contentSpan.end > style.contentSpan.start) {
      const first = lineOfOffset(starts, style.contentSpan.start);
      const last = lineOfOffset(
        starts,
        Math.max(style.contentSpan.start, style.contentSpan.end - 1),
      );
      for (let l = first; l <= last; l++) {
        out[l] = lineTextOf(l);
        kinds[l] = "verbatim";
    ...
    const tail = source.slice(
      Math.max(starts[close], style.contentSpan.start),
      style.contentSpan.end,
    );
    ...
    if (open !== close) {
      ...
      recompose(
        ...
        `${lead}<style>`,
        head,
        style.contentSpan.start - starts[open],
      );
  ...
  file.entities.forEach((entity, e) => {
    ...
    const buildChain = (
      ...
      scripts.forEach((sc, i) => {
        ...
        const closerLine = lineOfOffset(
          starts,
          Math.max(sc.span.start, sc.span.end - 1),
        );
        if (sc.contentSpan.end > sc.contentSpan.start)
          copyVerbatim(sc.contentSpan);
        ...
        if (i === 0) {
          ...
          if (openLine === closerLine) {
            ...
            if (langFix) {
              const lineEnd =
                openLine + 1 < total ? starts[openLine + 1] - 1 : source.length;
    ...
    const entityScriptLines = (): number[] => {
      ...
      const last = lineOfOffset(
        starts,
        Math.max(entityScript.span.start, entityScript.span.end - 1),
      );
      const lines: number[] = [];
    ...
    if (
      (entity.kind === "PAGE" || entity.kind === "LAYOUT") &&
      (entity.script || entity.style)
    ) {
      const out: string[] = new Array(total).fill("");
      const kinds: ProjectedLineKind[] = new Array(total).fill("blank");
      ...
      const scripts = [file.script, entity.script].filter(
    ...
    if (entity.kind === "DOC" && (entityScript || entity.style)) {
      ...
      const scripts = [file.script, entityScript].filter(
        (x): x is TagBlockLike => !!x,
      );
      let proseFrom = openLast + 1;
      if (scripts.length > 0) {
      ...
      entity.blocks.forEach((block, b) => {
        ...
        kinds[bOpen] = "wrapper";
        if (block.markupSpan.end > block.markupSpan.start)
          copyVerbatim(block.markupSpan);
    ...
    entity.blocks.forEach((block, b) => {
      if (!block.script && !block.style && !entityScript && !entityDocOwnsBody)
        return;
      ...
      if (scripts.length > 0) {
        ...
        if (block.script) {
          ...
          out[closeLine] =
            `${out[closeLine]}</script>{#snippet ${name}${argsParam}}`;
        } else {
