...
/**
 * Resolve CSS paths to absolute filesystem paths, and say so when one points
 * at nothing.
 *
 * A mistyped `css` is invisible otherwise: dev serves every stage unstyled and
 * blames nothing, so the natural conclusion is that the project's CSS does not
 * apply to previews — which is the one thing `css` exists to make happen. It
 * is a warning rather than an error because the file may be generated by a
 * build step that has not run yet.
 */
function resolveCssPaths(
	...
	if (!css) return null;
	const one = (href: string, label: string): string => {
		if (href.startsWith('http')) return href;
		const path = href.startsWith('/') ? href : resolve(root, href);
		if (!existsSync(path)) {
			console.warn(`[sdocs] \`css${label}\`: no file at ${path} — stages will render unstyled.`);
		}
		return path;
	};
	if (typeof css === 'string') return one(css, '');
	const resolved: Record<string, string> = {};
	for (const [name, href] of Object.entries(css)) {
		resolved[name] = one(href, `.${name}`);
	}
	return resolved;
}
