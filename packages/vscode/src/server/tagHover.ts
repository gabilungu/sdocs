...
const TAG_DOCS: Record<string, string> = {
	DOCS: [
		...
		'- `padding` / `direction` / `gap` — default stage layout for this entity\'s previews and examples (defaults `16px`, `row`, `16px`).',
		'',
	...
	preview: [
		...
		'- `maxWidth` / `padding` / `direction` / `gap` — stage layout overrides.',
		'',
		`[Component docs reference](${SITE}/language/component-docs)`,
	].join('\n'),
	example: [
